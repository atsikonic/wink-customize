...
<template>
    <div class="w-screen min-h-screen flex flex-col">
        ...
        <div class="flex flex-1 w-screen">
            ...
            <main class="container flex-1 w-full p-6">
                <page-header>
                ...
                <div v-if="ready && entries.length == 0 && !isFiltered">
                    ...
                    <router-link :to="{name:'post-new'}" class="no-underline text-primary hover:text-primary-dark">
                    ...
                    <div v-for="entry in entries" :key="entry.id" class="border-t border-very-light flex items-center">
                        <div class="py-4" :title="entry.title">
                            ...
                            <small class="text-light">
                                <span v-if="entry.published && !dateInTheFuture(entry.publish_date)">
                                    Published {{timeAgo(entry.publish_date)}}
                                </span>
                                <span v-if="entry.published && dateInTheFuture(entry.publish_date)" class="text-green">
                                    Scheduled {{timeAgo(entry.publish_date)}}
                                </span>
                        ...
                        <router-link :to="{name:'post-edit', params:{id: entry.id}}" class="no-underline ml-auto hidden lg:block">
                            <div class="w-16 h-16 rounded-full bg-cover" v-if="entry.featured_image" 
                                 :style="{ backgroundImage: 'url(' + entry.featured_image + ')' }"></div>
